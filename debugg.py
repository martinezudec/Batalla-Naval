...
def ship_placement_validation(start_row,end_row,start_column,end_column):
...
def place_ship_in_sea(row,column,direction,lenght):
    ...
    if direction=="left":
        if column-lenght<0:
            return False
        start_column=column-lenght+1

    elif direction=="right":
        if column+lenght>=sea_size:
            return False
        end_column=column+lenght
    elif direction=="up":
        if row-lenght<0:
            return False
    ...
    return ship_placement_validation(start_row, end_row, start_column, end_column)
...
def create_sea():
    ...
    rows,columns=(ships_num, ships_num)
    #creación del mar
    sea=[]
    for i in range(rows):
        row=[]
    ...
    while ships_num_placed !=ships_num:
        ...
        ship_size=3
        if place_ship_in_sea(random_row,random_column,direction,ship_size):
            ships_num_placed+=1
